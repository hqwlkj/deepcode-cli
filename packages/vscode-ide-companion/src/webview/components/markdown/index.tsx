...
const Markdown: FC<MarkdownProps> = ({ className, children = "", onFileClick, enableFileLinks }) => {
  ...
  const processFilePaths = (html: string): string => {
    ...
    const FILE_PATH_NO_G = new RegExp(FILE_PATH_REGEX.source, FILE_PATH_REGEX.flags.replace("g", ""));
    ...
    const normalizePathAndLine = (raw: string): { displayText: string; dataPath: string } => {
    ...
    const upgradeAnchorIfFilePath = (a: HTMLAnchorElement) => {
      ...
      if (httpMatch) {
        try {
          ...
          if (noPath && BARE_FILE_REGEX.test(text) && host.toLowerCase() === text.toLowerCase()) {
            const { dataPath } = normalizePathAndLine(text);
  ...
  const handleContainerClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      ...
      const anchor = (target.closest && target.closest("a.file-path-link")) as HTMLAnchorElement | null;
      if (anchor) {
      ...
      const anyAnchor = (target.closest && target.closest("a")) as HTMLAnchorElement | null;
      if (!anyAnchor) {
        return;
      }
